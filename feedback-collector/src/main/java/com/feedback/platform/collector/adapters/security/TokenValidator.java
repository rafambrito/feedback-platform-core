package com.feedback.platform.collector.adapters.security;
...
public class TokenValidator {
    ...
    public TokenValidator() {
        this(
                resolveRequiredValue(ENV_EXPECTED_ISSUER),
                resolveRequiredValue(ENV_EXPECTED_AUDIENCE),
                resolveRequiredValue(ENV_JWK_SET_JSON),
                Clock.systemUTC()
        );
    }
    ...
    private static String resolveRequiredValue(String key) {
        String fromSystemProperty = System.getProperty(key);
        if (fromSystemProperty != null && !fromSystemProperty.isBlank()) {
            return fromSystemProperty;
        }

        String fromConfig = ConfigProvider.getConfig().getOptionalValue(key, String.class).orElse(null);
        if (fromConfig != null && !fromConfig.isBlank()) {
            return fromConfig;
        }

        String fromEnv = System.getenv(key);
        return requireNonBlank(fromEnv, "Missing configuration value: " + key);
    }
